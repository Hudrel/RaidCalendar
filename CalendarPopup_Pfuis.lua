...
function M.new()
	...
	local function refresh_settings_labels()
		...
		popup.detail_panel.header:SetText( m.L( "ui.day_schedule" ) )
		popup.detail_panel.empty:SetText( m.L( "ui.no_events_day" ) )
	...
	local function update_day_detail_items()
		local day_events = get_selected_day_events() or {}
		local panel = popup.detail_panel

		panel.empty:Hide()
		panel.subheader:SetText( m.format_local_date( selected_day, "day_month_year_compact" ) )
		panel.header_count:SetText( m.L( getn( day_events ) == 1 and "ui.event_count_one" or "ui.event_count_many", { count = getn( day_events ) } ) )

		for i = 1, getn( detail_items ) do
			detail_items[ i ]:Hide()
		end

		if getn( day_events ) == 0 then
			panel.empty:Show()
			return
		end

		for i, item in ipairs( day_events ) do
			local event = m.db.events[ item.key ]
			...
			frame.time:SetText( date( m.time_format, event.startTime ) )
	...
	local function refresh_day_cells()
		...
		for i = 1, getn( day_cells ) do
			...
			cell.day_number:SetTextColor( is_current_month and 1 or 0.45, is_current_month and 0.82 or 0.45, is_current_month and 0 or 0.45 )
	...
	local function create_frame()
		...
		frame.detail_panel.header:SetText( m.L( "ui.day_schedule" ) )
		...
		frame.detail_panel.empty:SetText( m.L( "ui.no_events_day" ) )
