	...
	local function on_command( command, data, sender )
		...
		elseif command == MessageCommand.DiscordAuth then
			...
			if data.player == m.player then
				...
				-- Mettre à jour l'UI du calendar popup si visible
		...
		elseif command == MessageCommand.BotStatus then
			...
			if data.player == m.player then
				...
				-- Bust cache so bot_online_status() re-checks guild roster immediately
				if m.reset_bot_status_cache then m.reset_bot_status_cache() end
