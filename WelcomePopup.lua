...
function M.new()
	...
	local function start_timer()
		...
		popup.status1:SetText( m.L( "ui.checking_bot" ) )
		...
		popup:SetScript( "OnUpdate", function()
			...
			if now - timer >= 7 then
				...
				popup.status1:SetText( m.L( "ui.checking_bot_no_response" ) )
			end
		end )
	end
	...
	local function on_discord_check()
		...
		if string.find( discord_id, "^%d+$" ) then
			popup.status2:SetText( m.L( "ui.discord_user_found", { user_id = discord_id } ) )
			popup.status3:SetText( m.L( "ui.auth_request_sent", { bot_name = m.db.user_settings.discord_bot } ) )
	...
	local function on_close()
		StaticPopupDialogs["RC_WELCOME_CLOSE"] = {
			text = m.L( "ui.welcome_close_prompt" ),
			button1 = m.L( "actions.verify" ),
			button2 = m.L( "actions.close" ),
			OnAccept = function()
				popup:Hide()
			end,
			OnCancel = function()
	...
	local function create_frame()
		...
		label_welcome:SetText( m.L( "ui.welcome_title" ) )
		frame.label_welcome = label_welcome
		...
		label_info:SetText( m.L( "ui.welcome_info" ) )
		...
		label_status1:SetText( m.L( "ui.checking_bot" ) )
		...
		label_discord:SetText( m.L( "ui.discord_name_id" ) )
		...
		local btn_verify = gui.create_button( step2, m.L( "actions.verify" ), 70, on_discord_check )
		btn_verify:SetPoint( "Left", input_discord, "Right", 10, 0 )
		frame.btn_verify = btn_verify

		local btn_complete = gui.create_button( frame, m.L( "actions.close" ), 80, function() frame:Hide() end )
	...
	local function bot_response( name )
		if popup and popup:IsVisible() then
			...
			popup.status1:SetText( m.L( "ui.bot_response", { name = name } ) )
			popup.status2:SetText( m.L( "ui.enter_discord_username" ) )
			popup.step2:Show()
	...
	local function discord_response( success, user_id )
		if popup and popup:IsVisible() then
			if success then
				popup.status2:SetText( m.L( "ui.discord_user_found", { user_id = user_id } ) )
				popup.status3:SetText( m.L( "ui.auth_request_sent", { bot_name = m.db.user_settings.discord_bot } ) )
				m.msg.authorize_user( user_id )
			else
				popup.status2:SetText( m.L( "ui.no_user_id_found", { name = popup.input_discord:GetText() } ) )
				popup.btn_verify:Enable()
	...
	local function auth_response( user_id, success )
		if success then
			popup.status3:SetText( m.L( "ui.authorization_granted" ) )
			popup.step2:Hide()
			popup.btn_complete:Show()
		else
			popup.status3:SetText( m.L( "ui.authorization_denied" ) )
			popup.btn_verify:Enable()
			popup.input_discord:EnableKeyboard( true )
			popup.input_discord:EnableMouse( true )
		end
	...
	local function refresh_locale()
		...
		popup.label_welcome:SetText( m.L( "ui.welcome_title" ) )
		popup.label_info:SetText( m.L( "ui.welcome_info" ) )
		popup.label_discord:SetText( m.L( "ui.discord_name_id" ) )
		popup.btn_verify:SetText( m.L( "actions.verify" ) )
		popup.btn_complete:SetText( m.L( "actions.close" ) )
	end
